...
def Lst():
    ...
    with open(args.f) as file1:
        ...
        # Generar versión en texto plano (.LST) con formato clásico
        with open(f'./{args.f}.LST', 'w') as plain_lst:
            plain_lst.write("Número de línea:  código objeto:  código fuente (en una sola línea)\n\n")

            for x, i in enumerate(file1):  # si lo quieres desde 1: enumerate(file1, start=1)
                num = f"{x:>3}:"  # <- dos puntos pegados al número

                if info[x] != '' and not 'Etiqueta' in info[x]:
                    # Línea con instrucción
                    direccion = codigo[x][1]
                    opcode    = codigo[x][3]
                    operando  = codigo[x][4]
                    codigo_obj = (opcode + operando).strip()

                    # columnas: [num:] [dir] [obj] : [fuente]
                    linea = f"{num:<5} {direccion:<6} {codigo_obj:<12} : {i}\n"
                    plain_lst.write(linea)
                else:
                    # Líneas sin instrucción -> "Vacio"
                    linea = f"{num:<5} {'Vacio':<18} : {i}\n"
                    plain_lst.write(linea)
...
def s19():
    ...
    # Generar versión en texto plano (.S19)
    with open(f'./{args.f}.S19', 'w') as plain_s19:
        lineas = []
        direccion_actual = ""
        bytes_linea = []

        # Recorre el arreglo 'codigo' para reconstruir los bytes reales
        for ins in codigo:
            if ins != '' and ins[2] != 'Etiqueta':
                direccion = ins[1].upper()
                codigo_obj = (ins[3] + ins[4]).strip().upper()
                if not direccion_actual:
                    direccion_actual = direccion

                # Divide el código en bytes de 2 caracteres
                bytes_hex = [codigo_obj[i:i+2] for i in range(0, len(codigo_obj), 2) if codigo_obj]

                for byte in bytes_hex:
                    bytes_linea.append(byte)
                    # Cada 16 bytes, se cierra una línea y se reinicia
                    if len(bytes_linea) == 16:
                        lineas.append(f"<{direccion_actual}> " + " ".join(bytes_linea))
                        direccion_actual = f"{int(direccion_actual,16)+16:04X}"
                        bytes_linea = []

        # Si sobran bytes sin completar 16
        if bytes_linea:
            lineas.append(f"<{direccion_actual}> " + " ".join(bytes_linea))

        # Escribe todas las líneas
        for linea in lineas:
            plain_s19.write(linea + "\n")
...
if errores:
    ...
    with open(args.f) as file:
        ...
        # Evita desbordamiento si info tiene más elementos que file
        for cont in range(min(len(file), len(info))):
